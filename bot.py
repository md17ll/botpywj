...
def clean(t):
    if not t:return ""
    t=re.sub(r'[أإآٱ]','ا',t); t=t.replace('ؤ','و').replace('ئ','ي').replace('ء','')
    t=re.sub(r'[\u064B-\u065F\u0670\u06D6-\u06ED]','',t); t=t.replace('ـ','').replace('`','').replace('"','').replace("'","")
    t=re.sub(r'[*_~#]+','',t); t=re.sub(r'[ \t]+',' ',t); t=re.sub(r'\s*\n\s*',' ',t); return t.strip()
...
def generate(theme=None,style=None):
    chosen=random.sample(THEMES,random.randint(2,4)) if not theme else [theme]+random.sample([x for x in THEMES if x!=theme],2)
    sty=STYLES.get(style,random.choice(list(STYLES.values()))) if style else random.choice(list(STYLES.values()))
    old=q("SELECT text FROM posts WHERE status='published' ORDER BY id DESC LIMIT 12",fetch=True)
...
المطلوب جملة واحدة فقط، من 8 الى 22 كلمة تقريبا. ممنوع المقالات والفقرات والشرح والمقدمات والهاشتاغات.
لا تستخدم التشكيل. لا تضع عنوانا. لا تقل مقولة او عبارة. اكتب الناتج فقط.
مثال على الطول والروح فقط ولا تنسخه: الوحدة ليست غياب الناس، بل اللحظة التي تسمع فيها نفسك بوضوح.
ابتكر معنى جديدا ولا تكرر النصوص السابقة:\n{avoid}""")

def similar(text):
    rows=q("SELECT id,text FROM posts WHERE status='published' ORDER BY id DESC LIMIT 80",fetch=True); best=(0,None)
    for r in rows:
...
def publish(text,source="manual"):
    ...
    bot.send_message(CHAT_ID,t); now=datetime.now().isoformat(timespec="seconds")
    q("INSERT INTO posts VALUES(NULL,?,?,?,?,?,?,?,?,?)",(t,"short","","",source,"published",0,now,now)); return t
...
def mainkb():return kb([[("🧠 توليد عبارة","content")],[("🔄 النشر الدوري","auto"),("📅 خطة النشر","plan")],[("⏳ المجدولة","scheduled"),("🗃 الارشيف","archive"),("📊 الاحصائيات","stats")],[("📑 3 خيارات","quick3"),("📚 5 خيارات","quick5"),("💰 استهلاك AI","usage")],[("🟢 حالة البوت","status"),("⚙️ الاعدادات","settings")]])
...
def previewkb():return kb([[("📢 نشر الان","pub")],[("⏳ جدولة","schedule"),("🔄 عبارة جديدة","regen")],[("🧠 تحسين","improve"),("✏️ تعديل","edit"),("⭐ حفظ","save")],[("📑 3 خيارات","variants_3"),("📚 5 خيارات","variants_5")],[("👍 ممتاز","up"),("👎 لم يعجبني","down")],back()])
...
def text_input(m):
    ...
    try:
        if w=="customint":
            ...
            hh=m.text.strip(); datetime.strptime(hh,"%H:%M"); q("INSERT INTO slots(hhmm,ctype) VALUES(?,?)",(hh,"short")); st.pop("wait",None); bot.send_message(m.chat.id,"✅ اضيف الوقت "+hh,reply_markup=mainkb())
...
def cb(c):
    ...
    if d in ("home","go_home"):edit_screen(c,"🤖 لوحة العبارات الذكية",mainkb())
        ...
        st["style"]=None if d=="st_smart" else d[3:]; edit_screen(c,"⏳ جاري التوليد...",kb([back()])); st["text"]=generate(st.get("theme"),st.get("style")); edit_screen(c,"👁 معاينة نظيفة:\n\n"+st["text"],previewkb())
    ...
    elif d.startswith("variants_") or d in ("quick3","quick5"):
        n=3 if d in ("variants_3","quick3") else 5; opts=[generate(st.get("theme"),st.get("style")) for _ in range(n)]; st["variants"]=opts; edit_screen(c,"\n\n".join(f"{i+1}) {x}" for i,x in enumerate(opts)),kb([[(f"الخيار {i+1}",f"pick_{i}")] for i in range(n)]+[back()]))
    elif d.startswith("pick_"):st["text"]=st["variants"][int(d[5:])]; edit_screen(c,"👁 اخترت:\n\n"+st["text"],previewkb())
        ...
        r=q("SELECT * FROM schedules WHERE id=?",(int(d[6:]),),True)[0]; st["sid"]=r["id"]; edit_screen(c,r["text"],kb([[("📢 نشر الان","schedpub"),("❌ الغاء","schedcancel")],back()]))
