...
def ai_once(prompt,temp=.9):
    try:
        r=requests.post("https://openrouter.ai/api/v1/chat/completions",headers={"Authorization":f"Bearer {AI_KEY}","Content-Type":"application/json","HTTP-Referer":"https://railway.app","X-Title":"Telegram Content Manager"},json={"model":MODEL,"messages":[{"role":"user","content":prompt}],"temperature":temp,"max_tokens":120},timeout=(10,75))
        if r.status_code!=200:return "",f"OpenRouter HTTP {r.status_code}: {r.text[:160]}"
        d=r.json()
        if d.get("error"):return "","OpenRouter: "+str(d["error"].get("message","unknown"))
        choices=d.get("choices") or []
        if not choices:return "","OpenRouter لم يرجع choices"
        msg=choices[0].get("message") or {}; text=clean(msg.get("content") or "")
        if not text:return "","OpenRouter رجع محتوى فارغ"
        ...
        return text,""
    except Exception as e:return "","اتصال OpenRouter: "+str(e)[:140]

def ai(prompt,temp=.9):
    if not AI_KEY:return "خطا: مفتاح OpenRouter غير موجود"
    last=""
    for _ in range(2):
        text,last=ai_once(prompt,temp)
        if text:return text
    return "خطا AI: "+(last or "لم يتم توليد محتوى")

def generate(theme=None,style=None,smart=False):
    if smart or not theme:
        chosen=random.sample(THEMES,random.randint(3,5)); sty=random.choice(list(STYLES.values()))
    else:
        chosen=[theme]+random.sample([x for x in THEMES if x!=theme],2); sty=STYLES.get(style,random.choice(list(STYLES.values()))) if style else random.choice(list(STYLES.values()))
...
لا تستخدم التشكيل ولا عنوانا. اكتب الناتج فقط. لا تكرر قالب لغوي واحد في كل مرة.
...
def show_generated(c,st,label="👁 معاينة نظيفة:"):
    text=st.get("text","")
    if not text or text.startswith("خطا"):
        edit_screen(c,"❌ "+(text or "خطا AI: لم يتم توليد محتوى"),kb([[("🔄 حاول مجددا","regen")],back()])); return False
    edit_screen(c,label+"\n\n"+text,previewkb()); return True
...
def text_input(m):
    ...
    try:
        if w=="customint":
            h=float(m.text.replace(",",".")); assert h>0; sec=int(h*3600); ss("auto_interval",sec); ss("auto_enabled","1"); ss("next_auto",(datetime.now()+timedelta(seconds=sec)).isoformat(timespec="seconds")); st.pop("wait",None); bot.send_message(m.chat.id,f"✅ نشر تلقائي كل {h:g} ساعة - مزيج ذكي",reply_markup=mainkb())
        elif w=="sccustom":
...
def cb(c):
    ...
    if d in ("home","go_home"):edit_screen(c,"🤖 لوحة العبارات الذكية",mainkb())
        ...
        edit_screen(c,"⏳ انشاء عبارة ذكية ونشرها...",kb([back()])); t=generate(smart=True)
        if t.startswith("خطا"):edit_screen(c,"❌ "+t,kb([[("🔄 حاول مجددا","try_publish")],back()]))
        else:
            try:publish(t,"instant-smart"); edit_screen(c,"✅ تم انشاء العبارة ونشرها مباشرة:\n\n"+t,mainkb())
            except Exception as e:edit_screen(c,"❌ فشل النشر: "+str(e)[:160],mainkb())
        ...
        st["style"]=None if d=="st_smart" else d[3:]; edit_screen(c,"⏳ جاري التوليد...",kb([back()])); st["text"]=generate(st.get("theme"),st.get("style")); show_generated(c,st)
    elif d=="regen":st["text"]=generate(st.get("theme"),st.get("style")); show_generated(c,st,"👁 عبارة جديدة:")
    elif d=="pub":
        ...
        st["text"]=ai("حسن هذه العبارة مع الحفاظ على معناها. اجعلها جملة واحدة قصيرة جدا من 8 الى 22 كلمة، عميقة ومرتبة. لا توسعها ولا تشرحها. اكتب الناتج فقط:\n"+st.get("text","")); show_generated(c,st,"👁 المحسن:")
    ...
    elif d=="auto":edit_screen(c,"🔄 النشر الدوري دائما مزيج ذكي من كل المواضيع والاساليب\nاختر الفترة:",autokb())
    elif d.startswith("int_"):
        sec=int(d[4:]); ss("auto_interval",sec); ss("auto_enabled","1"); ss("next_auto",(datetime.now()+timedelta(seconds=sec)).isoformat(timespec="seconds")); edit_screen(c,f"✅ يعمل كل {sec/3600:g} ساعة - مزيج ذكي",mainkb())
    ...
    elif d=="status":edit_screen(c,f"🟢 البوت يعمل\nالنشر الدوري: {'يعمل - مزيج ذكي' if gs('auto_enabled')=='1' else 'متوقف'}\nAI: {'موجود' if AI_KEY else 'مفقود'}",kb([back()]))
    elif d=="settings":edit_screen(c,"⚙️ العبارات القصيرة مفعلة\n🎲 الدوري والخطة: مزيج ذكي دائما\n🧹 بدون تشكيل وهمزات\n📝 جملة واحدة قصيرة فقط",kb([back()]))
    elif d=="plan":
        rs=q("SELECT * FROM slots ORDER BY hhmm",fetch=True); edit_screen(c,"📅 خطة النشر - مزيج ذكي\n"+("\n".join(r['hhmm'] for r in rs) if rs else "لا توجد اوقات"),kb([[("➕ اضافة وقت","slotadd")],[("▶️ تشغيل","planon"),("⏸ ايقاف","planoff")],back()]))
    elif d=="slotadd":st["wait"]="slot"; edit_screen(c,"ارسل الوقت مثل 21:30",kb([back()]))
    elif d=="planon":ss("plan_enabled","1"); edit_screen(c,"▶️ تم تشغيل الخطة - مزيج ذكي",mainkb())
